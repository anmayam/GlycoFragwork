...
namespace GlypID
{
	...
	double clsIsotopeFit::GetFitScore(float (&mzs) __gc [], float (&intensities) __gc [], 
		GlypID::Peaks::clsPeak* (&peaks) __gc [], short charge, int peak_index, double delete_intensity_threshold,
		double min_intensity_for_score, System::Collections::Hashtable* elementCounts)
	{
		...
		GlypID::Utils::ConvertElementTableToFormula(mobj_fit->mobj_isotope_dist.mobj_elemental_isotope_composition, elementCounts, formula);
