...
namespace Engine
{
	namespace HornTransform
	{
		...
		bool MassTransform::FindPrecursorTransform(PeakProcessing::PeakData &pk_data, double parent_mz, std::vector<IsotopeFitRecord> *parent_transform_records, 
			double background_intensity, double min_peptide_intensity)
		{
			int mint_window_size = 5 ; 
			...
			std::vector <IsotopeFitRecord> vectTransformRecord ; 
			

			
			bool found_parent = pk_data.GetClosestPeak(parent_mz, 1, parentPeak) ; 			
			int parent_index = parentPeak.mint_peak_index ; 
			bool found_transform_record = false ; 

			bool found_peak = pk_data.GetNextPeak(minMZ, maxMZ, currentPeak) ;
			...
			while(found_peak)
			{
				if (currentPeak.mdbl_intensity < min_peptide_intensity)
					break ;					
				bool found_transform = FindTransform(pk_data, currentPeak, transformRecord) ;
				...
				found_peak = pk_data.GetNextPeak(minMZ, maxMZ, currentPeak) ;
			}
			...
			if (numDeisotoped == 1)
			{
				...
				for (int isotope_num = 0 ; isotope_num < num_isotopes && !found_transform_record; isotope_num++)
				{
					...
					{	
						...
						parent_transform_records->push_back(transformRecord) ; 
						break ; 
					}
				}		
			}
		...
		void MassTransform::SetElementalIsotopeComposition(const HornTransformTheoreticalProfile::AtomicInformation &iso_comp)
		{
			mobj_isotope_fitter->SetElementalIsotopeComposition(iso_comp) ; 
		}

		void MassTransform::GetElementalIsotopeComposition(HornTransformTheoreticalProfile::AtomicInformation &iso_comp)
		{
