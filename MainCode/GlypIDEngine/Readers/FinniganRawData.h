...
// also copy the following files into the bin folder in Engine folder :
//CFRDBResources.dll
//CFRUtil.dll
//FControl2.dll
//Fglobal.dll
 ...
 namespace Readers
{
	class  FinniganRawData : public RawData
	{
		...
		public:
			...
			bool IsSIDScan(int scan_num) ; 
			double GetAGCAccumulationTime(int scan_num) ; 
			double GetTICForScan(int scan_num) ; 	
			bool IsHCDScan(int scan_num); 
			bool IsETDScan(int scan_num) ; 
			bool IsCIDScan(int scan_num) ; 
